...
# =========================================================
# 2. SESSION STATE (OTURUM & ORTAK GLOBAL VERİLER)
# =========================================================
...
# Stok Veritabanı (Tüm personeller ortak görür)
...
# =========================================================
# 3. KULLANICI GİRİŞ (LOGIN) & ŞİFREMİ UNUTTUM EKRANI
# =========================================================
if not st.session_state.logged_in:
    ...
    with col2:
        if not st.session_state.sifremi_unuttum_mod:
            ...
            st.info("Kayıtlı kullanıcı adınızı girin ve sistem yöneticisi onaylı geçici şifre oluşturun.")
            
            with st.form("sifremi_unuttum_form"):
                unutulan_user = st.text_input("Kullanıcı Adınız")
                admin_onay_sifre = st.text_input("Yönetici (Admin) Şifresi", type="password")
                yeni_gecici_sifre = st.text_input("Yeni Şifreniz", type="password")
                
                sifirla_btn = st.form_submit_button("Şifreyi Sıfırla", use_container_width=True)
                geri_don_btn = st.form_submit_button("Giriş Ekranına Geri Dön", use_container_width=True)

                if sifirla_btn:
                    if unutulan_user in st.session_state.global_users:
                        if admin_onay_sifre == st.session_state.global_users.get("admin") or admin_onay_sifre == st.session_state.global_users.get("onur"):
                            if yeni_gecici_sifre:
                                st.session_state.global_users[unutulan_user] = yeni_gecici_sifre
                                st.success(f"✅ '{unutulan_user}' kullanıcısının şifresi başarıyla güncellendi!")
                                st.session_state.sifremi_unuttum_mod = False
                                st.rerun()
                            else:
                                st.error("❌ Yeni şifre boş olamaz!")
                        else:
                            st.error("❌ Yönetici (Admin) şifresi hatalı!")
                    else:
...
# =========================================================
# 4. SOL MENÜ & NAVİGASYON (ESKİ LİSTE)
# =========================================================
...
# =========================================================
    ...
    st.markdown(
        '<div class="page-subtitle">Depodaki mevcut mobilya ve malzeme stok seviyeleri (Tüm personeller ortak görür).</div>',
        unsafe_allow_html=True,
    )
    ...
    with st.form("stok_Guncelle_form"):
        ...
        if guncelle_btn:
            ...
            st.success(f"✅ {secilen_urun} stok hareketi işlendi ve tüm personellere yansıtıldı!")
            st.rerun()
...
# =========================================================
    ...
    st.markdown(
        '<div class="page-subtitle">Kataloğa yeni bir ürün ekleyin ve tüm personellerin görmesini sağlayın.</div>',
        unsafe_allow_html=True,
    )
    ...
    with st.form("yeni_urun_form"):
        ...
        if yeni_urun_onay:
            if not u_ad:
                ...
                st.success(f"✅ {u_ad} başarıyla sisteme tanımlandı ve tüm personellerin listesine eklendi!")
